...
def inference(dataset, model, ode_likelihood, batch_size, device, num_workers=0, batches=None, verbose=True, tw=None):
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    all_bpds = 0.
    all_items = 0
    score_id = []
...
def eval(dataset, reference_dataset, ood_datasets, model, ode_likelihood, batch_size, device, num_workers=0, recall=0.95, batches=None, verbose=True, tw=None):
    if verbose:
        print('Running eval. In-distribution data')
    score_id = inference(dataset, model, ode_likelihood, batch_size, device, num_workers, batches, verbose=verbose, tw=tw)
    if verbose:
        print('Running eval. Reference data')
    score_ref = inference(reference_dataset, model, ode_likelihood, batch_size, device, num_workers, batches, verbose=verbose, tw=tw)
    ...
    for i, ood_dataset in enumerate(ood_datasets):
        ...
        score_ood = inference(ood_dataset, model, ode_likelihood, batch_size, device, num_workers, batches, verbose=verbose, tw=tw)
...
def plot(eval_data, ood_names, model_name, data_name, out_dir='figs', config=None, verbose=True, main_label='Training'):
    if verbose:
    ...
    names = [f'{name}\nACU: {auc:.2f}\nFPR: {fpr:.2f}' for name, auc, fpr in zip([main_label]+ood_names, disp_auc, disp_fpr)]
...
def objective(config, trial, device):
    ...
    score = np.mean([np.mean(eval_data['score']) for eval_data in evals])
    score_min = np.min([np.mean(eval_data['score']) for eval_data in evals])
    score_max = np.max([np.mean(eval_data['score']) for eval_data in evals])
    score_ref = np.mean([np.mean(eval_data['score_ref']) for eval_data in evals])
    score_ref_min = np.min([np.mean(eval_data['score_ref']) for eval_data in evals])
    score_ref_max = np.max([np.mean(eval_data['score_ref']) for eval_data in evals])
    loss = np.mean([eval_data['train_loss'][-1] for eval_data in evals])
    loss_min = np.min([eval_data['train_loss'][-1] for eval_data in evals])
    loss_max = np.max([eval_data['train_loss'][-1] for eval_data in evals])
    auc = np.mean([np.mean(eval_data['auc']) for eval_data in evals])
    auc_min = np.min([np.mean(eval_data['auc']) for eval_data in evals])
    auc_max = np.max([np.mean(eval_data['auc']) for eval_data in evals])
    ref_auc = np.mean([eval_data['ref_auc'] for eval_data in evals])
    ref_auc_min = np.min([eval_data['ref_auc'] for eval_data in evals])
    ref_auc_max = np.max([eval_data['ref_auc'] for eval_data in evals])
    ref_fpr = np.mean([eval_data['ref_fpr'] for eval_data in evals])
    ref_fpr_min = np.min([eval_data['ref_fpr'] for eval_data in evals])
    ref_fpr_max = np.max([eval_data['ref_fpr'] for eval_data in evals])

    trial.set_user_attr('auc', float(auc))
    trial.set_user_attr('frp',float(ref_fpr))
    trial.set_user_attr('ref_auc', float(ref_auc))
    trial.set_user_attr('ref_fpr', float(ref_fpr))
    trial.set_user_attr('score', float(score))
    trial.set_user_attr('score_ref', float(score_ref))
    trial.set_user_attr('loss', float(loss))
    trial.set_user_attr('feat_dim', int(evals[0]['feat_dim']))
    trial.set_user_attr('encoder', cfg['encoder'])
    trial.set_user_attr('sigma', float(cfg['sigma']))
    trial.set_user_attr('lr', float(cfg['lr']))
    trial.set_user_attr('score_min', float(score_min))
    trial.set_user_attr('score_max', float(score_max))
    trial.set_user_attr('score_ref_min', float(score_ref_min))
    trial.set_user_attr('score_ref_max', float(score_ref_max))
    trial.set_user_attr('loss_min', float(loss_min))
    trial.set_user_attr('loss_max', float(loss_max))
    trial.set_user_attr('auc_min', float(auc_min))
    trial.set_user_attr('auc_max', float(auc_max))
    trial.set_user_attr('ref_auc_min', float(ref_auc_min))
    trial.set_user_attr('ref_auc_max', float(ref_auc_max))
    trial.set_user_attr('ref_fpr_min', float(ref_fpr_min))
    trial.set_user_attr('ref_fpr_max', float(ref_fpr_max))

    ref_auc = np.abs(ref_auc-0.5)
    return auc, ref_auc
...
def main_optuna():
    ...
    random.shuffle(jobs)
    parallelize(ask_tell_optuna, jobs, gpu_nodes, timeout=60*60*3)
